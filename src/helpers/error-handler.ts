...
export function handleClientSideErrors(error: any): boolean {
  ...
  switch (error.code) {
    ...
    case 13008:
      ...
      showMessage({
        style: "error",
        message: "Office is still working on the last operation. When it completes, try this operation again.",
      });
      return invokeFallBackDialog;
    case 13010:
      // Only seen in Office on the Web.
      showMessage({ style: "error", message: "Follow the instructions to change your browser's zone configuration." });
      return invokeFallBackDialog;
    default:
